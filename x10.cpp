...
void x10::writeXTBIIR(byte ModeCode, byte numberCode, int numRepeats) {
  ...
  byte Key_Sequence9 = B1001;		// 9
  byte Key_Sequence8 = B0001;		// 8
  byte Key_Sequence2 = B0100;		// 2
  ...
  for (int i = 0; i < numRepeats; i++) {
  	// send the three parts of the command:
  sendBits(startCode, 4, true);
  sendBits(Key_Sequence9, 4, false);
  sendBits(Key_Sequence8, 4, false);
  sendBits(Key_Sequence2, 4, false);
